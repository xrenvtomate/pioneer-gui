...
export default ({dronesToConnect, setDronesToConnect, setDrones}) => {
  ...
  return <Dialog.Root open={open} onOpenChange={setOpen}>
    <Dialog.Trigger asChild>
      <button onClick={fetchNets} className='bg-zinc-600 px-4 py-0.5 rounded transition-all hover:bg-slate-600 active:bg-slate-700 shadow hover:shadow-lg border border-white/10'>Выбрать хост</button>
    </Dialog.Trigger>
    <Dialog.Portal>
      ...
      <Dialog.Content className="rounded-xl fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-zinc-800 p-8 w-96 border border-white/10">
        ...
        <div className='flex flex-col gap-2'>
          ...
          {net}
          <button className='ml-auto' onClick={() => connect(net)}>Подключиться</button>
          </div>)}
        </div>
